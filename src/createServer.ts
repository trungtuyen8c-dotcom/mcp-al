...
import { z } from "zod";
import { apiGet, ApiError, type ApiContext } from "./apiClient.js";
...
// Tạo 1 McpServer mới gắn với ctx (apiKey + baseUrl) riêng của người gọi.
// Dùng chung cho stdio (1 ctx cố định từ env) và HTTP remote (1 ctx mới mỗi request, theo header).
export function createMcpServer(ctx: ApiContext): McpServer {
  ...
  server.tool(
    ...
    async ({ status, source, dateFrom, dateTo, limit }) => {
      try {
        const rows = await apiGet<OrderRow[]>(ctx, "/orders", { source });
  ...
  server.tool(
    ...
    async ({ code }) => {
      try {
        ...
        const detail = await apiGet<Record<string, unknown>>(ctx, `/orders/${lookup.order.id}`);
        return text(detail);
      } catch (err) {
        return errorText(err);
      }
  ...
  server.tool(
    ...
    async ({ search, limit }) => {
      try {
        const rows = await apiGet<CustomerRow[]>(ctx, "/customers");
  ...
  server.tool(
    ...
    async ({ customer, stockOnly, limit }) => {
      try {
        const rows = await apiGet<TrackingRow[]>(ctx, "/trackings", {
          customer,
  ...
  server.tool(
    ...
    async ({ report, params }) => {
      try {
        const data = await apiGet<unknown>(ctx, REPORTS[report], params);
